namespace Curiologix
{
    ...
    public class FortuneWheel : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            Coins = PlayerPrefs.GetInt(COINS_COUNT, 0);
        ...
        IEnumerator RotateWheel(float time, float maxAngle)
        {
            ...
            Debug.Log(FortuneWheelConfig.Instance.prizes[SelectedReward]);
            GR_SaveData.instance.Coins += FortuneWheelConfig.Instance.prizes[SelectedReward];
            Debug.Log(GR_SaveData.instance.Coins);
            if (FindObjectOfType<Handler>())
            {
                FindObjectOfType<Handler>().LoadInterstitialAd();
            }
